...
plot.overlap.l.connected <- function(a65,a70,a75,a80,scaffold=TRUE) {
	a65[,3] <- log(a65[,3],base=10); a70[,3] <- log(a70[,3],base=10); a75[,3] <- log(a75[,3],base=10); a80[,3] <- log(a80[,3],base=10)
	maxL <- max(a65[,2],a70[,2],a75[,2],a80[,2])
	maxC <- max(a65[,3],a70[,3],a75[,3],a80[,3])
	...
	if (scaffold==TRUE) {
		plot(1,1,col="white",xlim=c(0,maxL),ylim=c(0,maxC),xlab="Scaffold length",ylab="log10 scaffold count",main="Scaffold length distribution")
	} else {
		plot(1,1,col="white",xlim=c(0,maxL),ylim=c(0,maxC),xlab="Contig length",ylab="log10 contig count",main="Contig length distribution")
	}
	for (i in 1:dim(a65)[1]) {
	...
	lines(a65x,a65y,col="red",lwd=2)
	lines(a70x,a70y,col="green",lwd=2)
	lines(a75x,a75y,col="blue",lwd=2)
	lines(a80x,a80y,col="orange",lwd=2)
